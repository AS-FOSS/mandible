...
| A wrapped usage line **joins into one entry**; only a `usage:`/`or:` marker starts a new one | `mandible-extract/src/help_text/sections.rs` | [M-10]'s shape again, one level up: `git --help`'s one synopsis wraps across five physical lines, and storing one `usage` entry per source line let the detail pane's `usage_signature` (which prepends the node name to any entry not already starting with it) render the wrapped tail as a complete-looking fifth invocation, `git [--config-env=<name>=<envvar>] <command> [<args>]` — git never documented that. Fixed by Tier B rule 2's principle applied to the usage block: a line more indented than the block's base indent continues the entry above it; a `usage:`/`Usage:`/`or:` marker always starts a new one regardless of indent. The `or:` check is load-bearing in the other direction — `du --help`'s `  or:  du [OPTION]... --files0-from=F` is indented *more* than its block's base indent, so a rule that joined on indentation alone would merge it into the form above, losing a real second invocation. `corpus/git/2.43.0` (one entry) and `corpus/du/9.4` (two) are the regression net for each direction. |
