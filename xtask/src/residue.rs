...
//! hindsight. See spec §13.1d, where this non-decision is recorded
...
pub fn run(
    ...
    println!(
        "\nNothing below is a score. See xtask/src/residue.rs's doc comment and spec §13.1d.\n"
    );
...
mod tests {
    ...
    /// and spec §13.1d) is that no pass/fail path may ever consult a
        ...
        for (name, src) in [
            ...
            assert!(
                ...
                "{name} calls into `residue`: a discovery instrument must never be \
                 reachable from a gate (spec §13.1d)"
            );
        }
    }
