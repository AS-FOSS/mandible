...
# The shell completions are generated by build.rs into the build script's
# OUT_DIR, whose path contains a per-build hash that is not known ahead of
# time. Find it, and install each file under the name the shell actually
# looks for — bash-completion loads `completions/<command>`, so the file
# must be named `mandible`, not `mandible.bash`.
outdir="$(dirname "$(find target/release/build -name mandible.bash -print -quit)")"
test -n "${outdir}" -a -d "${outdir}"
install -Dpm0644 "${outdir}/mandible.bash" %{buildroot}%{_datadir}/bash-completion/completions/%{name}
install -Dpm0644 "${outdir}/_mandible"     %{buildroot}%{_datadir}/zsh/site-functions/_%{name}
install -Dpm0644 "${outdir}/mandible.fish" %{buildroot}%{_datadir}/fish/vendor_completions.d/%{name}.fish
