...
pub fn flatten(
    ...
    match matching_paths {
        Some(matches) if !matches.is_empty() => {
            push_filtered(root, root_path, 0, expanded, matches, pending, &mut out);
        }
        _ => push_plain(root, root_path, 0, expanded, pending, &mut out),
    }
    out
}
...
fn push_plain(
    ...
    if is_expanded {
        for child in &node.subcommands {
            ...
            push_plain(child, child_path, depth + 1, expanded, pending, out);
        }
    }
}
...
mod tests {
    ...
    fn root_collapsed_shows_only_root() {
        let root = tree();
        let rows = flatten(&root, &HashSet::new(), None, &HashSet::new());
    ...
    fn expanding_root_shows_direct_children_only() {
        ...
        let rows = flatten(&root, &expanded, None, &HashSet::new());
    ...
    fn filter_force_expands_ancestor_chain_to_a_match() {
        ...
        let rows = flatten(&root, &HashSet::new(), Some(&m), &HashSet::new());
    ...
    fn empty_matches_behaves_like_no_filter() {
        ...
        let with_empty = flatten(&root, &expanded, Some(&empty), &HashSet::new());
        let without = flatten(&root, &expanded, None, &HashSet::new());
