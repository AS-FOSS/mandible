...
- Detail-pane polish for the sectioned layout: a group divider's rule is drawn one shade lighter than the section header's — a named color one step up from the muted base with the lightening over it, so the rule reads as furniture rather than as murk against the background — and is dropped altogether where the divider opens its section, so a header and the group directly beneath it no longer read as one doubled line; list sections lose their uniform left margin, with single-character short spellings starting at the pane's content edge and every long spelling starting one short prefix (`-X, `) in whether or not a short precedes it, so the long names run down a single column instead of two; and a value placeholder is measured as part of the spelling it follows rather than given an aligned slot of its own, so `grep`'s `-e, --regexp PATTERNS` keeps its description on its own row instead of hanging it under a mostly empty line; list sections now share one description layout at every pane width, with every description line — first and continuation alike — beginning at the section's own column, a row whose spelling and placeholder reach that column starting its first line one space past them and returning to the column beneath, and a pane too narrow for the column bringing the column down rather than switching layouts; and POSITIONALS rows are inset two columns from the content edge, where a run of bare names used to sit flush against the pane border.
