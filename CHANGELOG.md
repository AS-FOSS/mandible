...
- The detail pane is now one scrollable document of sections — DESCRIPTION, USAGE, POSITIONALS, FLAGS, MODIFIERS, ENVIRONMENT, in that order and only when non-empty — with the entity count in each list section's header (`FLAGS (41)`), groups drawn as a full-width mixed-case divider rule whose rows stay at the section's own margin, inherited flags as the final such group rather than a section of their own, and a spelling column computed per section and fitted to the majority rather than the widest row, so an unusually wide entity hangs its description on the next line at a small fixed indent instead of pushing every other description right; a wrapped entry stays one row for selection and scrolling, and a search that selects a flag near the end of a long list no longer scrolls past the end of the pane's content.

- Detail-pane polish for the sectioned layout: a group divider's rule is drawn one shade lighter than the section header's and is dropped altogether where the divider opens its section, so a header and the group directly beneath it no longer read as one doubled line; list sections lose their uniform left margin, with single-character short spellings starting at the pane's content edge and every long spelling starting one short prefix (`-X, `) in whether or not a short precedes it, so the long names run down a single column instead of two; and a value placeholder is measured as part of the spelling it follows rather than given an aligned slot of its own, so `grep`'s `-e, --regexp PATTERNS` keeps its description on its own row instead of hanging it under a mostly empty line, while a section where the 45% width cap excludes more than half the rows now stacks rather than aligning on a column the remaining minority chose.
