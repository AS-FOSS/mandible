...
- A usage line's own trailing run of two or more operands now reaches the tree with each operand's own optionality and repetition preserved (`mandible xfs_mkfile`, `mandible unmkinitramfs`, `mandible applydeltarpm`, `mandible logsave`), instead of the tail-operand recovery still capping at one; a flag paired with an angle-bracket metavar or a brace-value placeholder glued with no space no longer blocks recovery either, so `mandible filefrag`, `mandible xdriinfo`, `mandible claude`, `mandible nft`, and `mandible gdk-pixbuf-pixdata` now show their own trailing operands too (docs/shapes.md S-041).
- `mandible ar` now shows two separate usage forms and all four of its trailing operands (`member-name`, `count`, `archive-file`, `file`), instead of a second alternate-form line spelling the tool as a full path (`/usr/bin/ar`) getting joined onto the first and swallowing both: an alternate spelling of the tool's own name — a full path or a dotted stem, matching `vim` against a resolved `vim.basic` — now opens a new usage entry the same way an exact name repeat already did (docs/shapes.md S-108).
...
- `mandible bpftrace`/`mandible node`/`mandible fzf` now show a tool's own documented environment variables (`BPFTRACE_BTF`, `NODE_DEBUG`, `FZF_DEFAULT_COMMAND`, …) in an `ENVIRONMENT` section of their own: recovered only from a row under an explicitly labeled environment heading (`ENVIRONMENT:`, `Environment variables:`, …), never scavenged from `ALL_CAPS` words in prose or usage placeholders like `PATH`/`FILE`/`TERM` — a tool that documents its env vars only in its man page correctly gets no `ENVIRONMENT` section here.

- `mandible ar` now shows the seventeen single-letter modifiers `ar` documents (`[a]`, `[D]`, `[l <text> ]`, …) in a `MODIFIERS` section of their own, instead of dropping every one of them: a modifier is a letter typed glued to an operation letter rather than a flag or a subcommand, so it was recognized as neither — and `ar`'s own `command specific modifiers:` heading contains the word "command", which sent its rows to the subcommand path to be discarded one by one for not looking like command names. The recognizer reads the row, not the heading (a bracket, exactly one letter, an optional operand, then a real separator and a description, twice in a row), so `llvm-ar`'s differently formatted `MODIFIERS:` table is read by the same rule with nothing keyed on either tool, and a bracketed reference footnote is refused as one.
...
- Shell completion now offers tool names where mandible expects a tool: the `TOOL` positional and the values of `--doctor` and `--report` complete to the commands on `$PATH`, so `mandible gi<TAB>` proposes `git` rather than the contents of the current directory. Delivered by the existing `--completions <shell>` generator, so every packaging channel picks it up with no per-channel file; zsh and fish carry it today, while bash keeps its previous filename fallback because clap_complete's ahead-of-time bash generator has no command-name completion to emit.
...
- `mandible`'s `--help` parser now keeps every alias a flag row documents instead of only the first short and first long spelling it recognized (issue #30's emission half): `tar --help`'s `-A, --catenate, --concatenate` keeps all three; `jdeprscan`/`jmod`/`lsof`'s `-?, -h, --help` keep all three; `dpkg`'s `--no-act, --dry-run, --simulate` keeps all three. A tool's own `-r[esolve]`-style abbreviation bracket (a prefix a user may type, glued to the rest of the word it abbreviates — one letter or more, short or long dashes: `ip`'s `-r[esolve]`/`-rc[vbuf]`, `--br[ief]`) is now read as one spelling naming the full word, rendered back exactly as documented, rather than either discarding the bracket or misreading it as a fabricated value. `mandible ffplay`'s (and `ffmpeg`'s, `ffprobe`'s) own `-h topic` / `-? topic` / `-help topic` / `--help topic` — the same issue's primary example, four physical rows for one option — now recovers `-help`'s own `topic` value instead of leaving it a valueless boolean: `-help`'s value is restored only when a run-mate sharing its description already carries the value cleanly *and* the raw document still contains `-help topic` as a literal phrase, never invented from a shared description alone (an early version of this fix also merged such rows into one multi-spelling entity; a full-`PATH` sweep found that too readily fired on unrelated flags sharing boilerplate commentary — `as`'s `-w`/`-X`, both "ignored"; `lto-dump`'s `-C`/`-CC`, both "[disabled]", repeated 505 times in that one document — so the merge was dropped and only the narrower, evidence-anchored value recovery ships). `-h` and `--help` still unify through the existing short/long alias pairing (spec §4.4); `-?` and `-help` are correctly separate entities.
...
- `mandible ffplay`'s (and every tool's) `--help`-shaped spelling could disappear entirely on the very first TUI open, not just merge with the wrong alias: `Runner::fill_node`'s own contract passes the already-extracted node back in as a merge candidate on every call, which routes even a tool's first load through `mandible-core/src/merge.rs`'s cross-source identity merge — not only a genuine second source, contrary to what a single-source extraction looks like it should trigger. That merge's `entity_identity` keyed a flag's long-like spelling on its bare name alone, so `-help` (single-dash) and `--help` (double-dash) collided into one bucket and the two-slot short/long reconstruction inherited from before `Entity::spellings` existed kept only one of the two dash counts, silently dropping the other — this is what actually produced `ffplay`'s reported `-h, -help topic` + `-? topic` with `--help` missing outright, not a stdout/stderr capture divergence as first suspected (the captured-fixture replay showed all three entities correctly the whole time). The identity key now carries dash count (`-help` keys `L:1:help`, `--help` keys `L:2:help`), so two genuinely different spellings of the same word are never bucketed together — only two sources that actually document the identical spelling are.
...
- `mandible pppdump` no longer merges `-h` and `-p[d]` into one fabricated flag: its usage synopsis writes `[-h | -p[d]]`, a `|`-alternation between two flags with different meanings (`-h` dumps in hex, `-p[d]` dumps printable and/or both), and the bracket-group pairing rule that correctly folds a genuine `[-v | --version]` alias pair into one flag was pairing this shape too, because a single-dash abbreviation-bracket spelling (`-p[d]`) reads as long-like by the same rule every other consumer of a flag's spellings uses; the pairing rule now requires the long side to be a genuine double-dash spelling, so `-h` and `-p[d]` (correctly named from its bracket) are recovered as the two separate flags they are.

- A flag's spelling list can no longer repeat itself (`mandible ld`'s own `-d, -d`, `mandible jdb`'s own `-?, -h, --help, -h`, and similar on ten other tools): the alias loop now refuses to record a spelling identical to one already collected in the same row and stops the run there instead, leaving the honest-incompleteness fallback (no invented value) to describe the rest rather than fabricating a second reading of a name already read once — GNU `sort --help`'s own `-c, --check, --check=diagnose-first` restates `--check` with an example value, not a second alias, and a bracket-less multi-letter run (`jdb`'s `-help`) currently truncating to its first character in the short-flag reader can coincidentally collide with a spelling already read; recognizing `-help` as its own four-letter spelling remains unresolved and is not attempted here.
...
- `mandible jmod`/`mandible llvm-ar`/`mandible mariadb-check` (and its `mariadb-repair`/`mariadbcheck` aliases) no longer fabricate a phantom flag from a table's own header-underline row (a run of 3+ dashes with no name character, e.g. jmod's `------  -----------`), `mandible jmod --target-platform` now carries its wrapped `<String: target-platform>` value name whole instead of truncating it at the line wrap and dumping the tail into the description, the `@<filename>`/`@<file>` "read options from a file" row (jmod, jlink, size) no longer corrupts the flag documented directly above it, and `mandible llvm-ar --format`/`--rsp-quoting` now show their `=default`/`=gnu`/… enumerated values in `choices` instead of dumping them into the description.
...
- The alias loop no longer folds a worked usage example naming two different flags into one fabricated multi-spelling entity (`dpkg-split`'s own `-a|--auto -o <complete> <part>`, `screen`'s own `-D -RR`, `xxd`'s own `-r -s off ...`): a bare-whitespace continuation is now refused once this row has already used an explicit `,`/`|` separator, and — once the previous spelling was a genuine one-letter short — refused unless the continuation is itself a genuine one-letter short with no bare value trailing it, while `iptables`'s real long-then-short pairs and `jdeprscan`'s real short-then-short-then-long row (both genuine one-flag aliases, no comma anywhere) keep merging exactly as before. Separately, in alias position — right after an explicit separator — a multi-letter unbracketed single-dash run (`gold`'s own `-G, -shared`) is now read as a spelling in its own right instead of the short-flag reader's truncate-to-first-character fallback misreading it as `-s` and colliding with `gold`'s genuinely different `-s, --strip-all`.
...
- The README and every release body now list the COPR repository (`dnf copr enable as-foss/mandible`, then `dnf install mandible`) among the install channels, which covers Fedora and, through the same project's EL chroots, EPEL.
...
- The parsed view and the raw `--help` view (`t`) now each remember their own scroll position, vertically and horizontally, and pressing `t` puts the view being entered back exactly where it was last left, replacing 0.4.5's proportional carry: scrolling one view no longer moves the other, a view not yet scrolled for the selected node opens at the top-left instead of at a fraction of the other view's extent, and a remembered position is clamped to the content the view actually has when it comes back; changing the selected node still starts both views at the top, and a keypress that cannot change it — `j` on the last row, `k` on the first, either on a single-node tree, a click on the row already selected, or collapsing the row the pane is already describing — now leaves both views' positions and any flag target untouched instead of throwing a document the reader had scrolled minutes into back to the top with nothing on screen to explain why.

- A repeatable positional now renders with the POSIX synopsis ellipsis the parser read it from (`FILE...`), so `grep`'s repeatable `FILE` is no longer indistinguishable from a positional taking exactly one; the ellipsis is measured as part of the row's head like any other part of a spelling, and repeatable flags are unaffected — `-v -v -v` says the same thing by being accepted again, not with a suffix.

- A description keeps the structure its author marked while its prose still reflows to the pane's width: a line indented deeper than its own paragraph, a list row (`- `, `* `, `+ `, `• `, `1. `, `1) `) and an `Example:`/`e.g.` line followed by command-shaped text each keep their line break and relative indentation, so `grep`'s `Example: grep -i 'hello world' menu.h main.c` is no longer smeared into the sentence after it, while ordinary hard-wrapped prose — including a uniformly indented block, which the old leading-whitespace test refused to unwrap — still joins into one paragraph and re-wraps cleanly; blank lines in a tool's own help now survive as paragraph breaks (the `--help` parser used to flatten them to a space before the sanitizer could see them), and the detail pane wraps each preserved line on its own at its own indent.

- USAGE synopses now reproduce the layout the tool itself drew rather than having every whitespace run collapsed to a single space on the way into the IR: LVM's `[    --reportformat basic|json ]` keeps the four columns that put its long under the longs of its short-and-long siblings, a tab in `pastebinit`'s `Usage:` line is still expanded to 8-column stops rather than passed through, and each alternative invocation form keeps the indentation its author gave it — compensated for the `Usage: ` label the heading makes redundant, so `ip`'s and `ssh-keygen`'s forms stay lined up under each other exactly as drawn, a form the author set deeper stays deeper, and one indented less than the label it was drawn against (`du`'s `  or:`) clamps at the left edge instead of going negative; the verbatim fallback a node degrades to when nothing parsed now takes that same tier, so `ar`'s padded command table reaches the pane as `  m[ab]        - move file(s) in the archive` rather than collapsed to single spaces and shoved against the left edge — the one view whose whole purpose is showing the author's document was quietly reformatting it — and it scrolls sideways under `[ui] horizontal_scroll` like the raw view it is; descriptions are unaffected and still reflow to the pane's width.
...
- The detail pane is now one scrollable document of sections — DESCRIPTION, USAGE, POSITIONALS, FLAGS, MODIFIERS, ENVIRONMENT, in that order and only when non-empty — with the entity count in each list section's header (`FLAGS (41)`), groups drawn as a full-width mixed-case divider rule whose rows stay at the section's own margin, inherited flags as the final such group rather than a section of their own, and a description column computed per section and fitted to the majority rather than the widest row, so an unusually wide entity starts its own first description line past its spelling instead of pushing every other description right; a wrapped entry stays one row for selection and scrolling, and a search that selects a flag near the end of a long list no longer scrolls past the end of the pane's content.

- Detail-pane polish for the sectioned layout: a group divider is drawn label-first like the section header above it, and the pane's furniture now reads as three clearly separated neutral steps — the pane borders at the terminal's own foreground, the section header a step below them, the group divider a step below that — drawn from the xterm-256 gray ramp, since the sixteen named colors offer only one step below a default foreground where two are needed, with both levels falling back to `DarkGray` on a terminal that does not announce 256 colors and nothing anywhere relying on dimming, which several terminals ignore; every heading and divider draws its label in exactly its own rule's style, same color and never bold, so a label can no longer read at a different weight from the line running out of it; and a divider's rule is dropped altogether where it opens its section, so a header and the group directly beneath it no longer read as one doubled line; list sections lose their uniform left margin, with single-character short spellings starting at the pane's content edge and every long spelling starting one short prefix (`-X, `) in whether or not a short precedes it, so the long names run down a single column instead of two; and a value placeholder is measured as part of the spelling it follows rather than given an aligned slot of its own, so `grep`'s `-e, --regexp PATTERNS` keeps its description on its own row instead of hanging it under a mostly empty line; list sections now share one description layout at every pane width, with every description line — first and continuation alike — beginning at the section's own column, a row whose spelling and placeholder reach that column starting its first line one space past them and returning to the column beneath, and a pane too narrow for the column bringing the column down rather than switching layouts; POSITIONALS rows are inset two columns from the content edge, where a run of bare names used to sit flush against the pane border; and the pane's vertical spacing now follows its container hierarchy — two blank rows above every section header, one above every ruled group divider, none below either (the heading's own rule is the separator) and none above the first header on the page — each count exact rather than a minimum however the block above happened to end, replacing a layout that gave section boundaries a single row and group boundaries none at all, so a dense list ran its groups together with nothing but a rule between them.
### Fixed

- `nix run github:AS-FOSS/mandible`, the command the README prints, failed for everyone with "cannot write modified lock file of flake": `flake.lock` was never committed, so every remote evaluation tried to resolve `nixpkgs` and write the lock into a store path it cannot write — the lock is now tracked, which also pins the `nixpkgs` revision the flake builds against instead of leaving it to whatever `nixpkgs-unstable` points at that day. The `nix` CI job could not see this because it evaluates a checkout, where nix writes the lock file locally and moves on.
- The rpm's and deb's shell completions were installed under build-hash paths and never loaded — every rpm shipped all three as `…/completions/mandible/mandible-<hash>/out/mandible.bash` and the deb did the same for bash, a directory where each shell looks for a file — because the packaging manifests globbed them out of the build script's hashed `OUT_DIR` and both packagers append a glob's matched path to the destination; completions are now generated from the built binary's own `--completions` by `scripts/gen_completions.sh`, the same generator the nix and Homebrew packages already used, and installed by literal path to `/usr/share/bash-completion/completions/mandible`, `/usr/share/fish/vendor_completions.d/mandible.fish` and, for zsh, the directory each distribution's zsh actually has on its default `fpath` — `/usr/share/zsh/vendor-completions/_mandible` in the deb and `/usr/share/zsh/site-functions/_mandible` in the rpm, since neither distribution carries the other's.
