...
- USAGE synopses now keep the spacing the tool itself printed instead of having every whitespace run collapsed to a single space on the way into the IR: LVM's `[    --reportformat basic|json ]` keeps the four columns that put its long under the longs of its short-and-long siblings, `sg_dd`'s and `renice`'s alternative forms keep the columns they were aligned in, and a tab in `pastebinit`'s `Usage:` line is still expanded to 8-column stops rather than passed through — descriptions are unaffected and still reflow to the pane's width.
