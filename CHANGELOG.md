...
- LVM's own `--help` emitter (`vgck`, `vgchange`, `lvextend`, `lvresize`, `pvscan`, and the rest of the `lv*`/`vg*`/`pv*` family) writes one stanza per operation mode — a prose description, its own `<tool> <args>` head, then TAB-indented `[ ... ]` rows — separated by blank lines, and the usage-block loop in `sections::parse_body` ended unconditionally on the first blank line, so only the first stanza was ever read (`vgck --updatemetadata`, a flag rather than a subcommand, was completely absent from the tree); the loop now looks one line past a blank line for the same synopsis-head evidence that opens the block in the first place, admitting a further stanza only when it is unambiguous, never reopening on a later paragraph that merely happens to repeat the tool's name (`corepack`'s and `pydoc3`'s own headingless-invocation and wrapped-description shapes are explicitly guarded against) — measured over the 2,301 frozen captures in `audit/queue-captures/`, this recovers `vgck`'s `--updatemetadata`, `pvscan`'s `--cache`/`--listvg`/`--listlvs`/`-a|--activate` family, `lvextend`'s `--usepolicies`, `adduser`'s `--system`/`--group`, and 2 real flags for `pydoc3`, with zero flags lost on a full-`PATH` sweep (a coarser raw flag-count dip on `lvextend`/`pvscan` is a pre-existing duplicate-flag defect being corrected, not a loss — full measurement in the fix's PR description).
