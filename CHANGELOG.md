...
- [S-174] A value spec with one bracket nested inside another keeps its whole spec now even when more of the outer group follows the inner pair's close, generalizing S-119 (`mandible fzf`'s `--listen`).
...
- [S-131, S-132] A usage-synopsis flag whose value name holds a space (`-w Process ID`) keeps the whole name now, and the trailing bracket group it sat beside no longer loses its own two-word positional (`mandible caffeinate`).
- [S-135] A usage line's own tab-indented, unpunctuated continuation sentence no longer folds into the usage text, letting the trailing positional it was blocking reach the tree (`mandible makeconv`).
...
- The detail pane's USAGE section no longer shows a continuation form's own `or:` marker or a doubled tool name (`mandible vim.basic`, `mandible cp`, `mandible ar`, `mandible du`).
