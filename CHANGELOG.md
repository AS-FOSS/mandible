...
- Detail-pane polish for the sectioned layout: a group divider is drawn label-first like the section header above it, with the section header's rule taking the brighter of the pane's two shades and the divider's the darker, and the divider's label taking the same shade as its own rule — both plain named colors rather than one of them dimmed, so the two levels keep their order on the terminals that ignore dimming, where the dimmed rule had been rendering brighter than the boundary it sits under — and the divider's rule is dropped altogether where it opens its section, so a header and the group directly beneath it no longer read as one doubled line; list sections lose their uniform left margin, with single-character short spellings starting at the pane's content edge and every long spelling starting one short prefix (`-X, `) in whether or not a short precedes it, so the long names run down a single column instead of two; and a value placeholder is measured as part of the spelling it follows rather than given an aligned slot of its own, so `grep`'s `-e, --regexp PATTERNS` keeps its description on its own row instead of hanging it under a mostly empty line; list sections now share one description layout at every pane width, with every description line — first and continuation alike — beginning at the section's own column, a row whose spelling and placeholder reach that column starting its first line one space past them and returning to the column beneath, and a pane too narrow for the column bringing the column down rather than switching layouts; POSITIONALS rows are inset to the long-flag column rather than to a smaller number of their own, so a bare positional name starts exactly where a preindented long starts in FLAGS instead of one column shy of it, where a run of bare names used to sit flush against the pane border; and exactly one blank row now separates a section's last row from the next section's header everywhere, however the section above happened to end.
