...
- `mandible`'s `--help` parser now keeps every alias a flag row documents instead of only the first short and first long spelling it recognized (issue #30's emission half): `tar --help`'s `-A, --catenate, --concatenate` keeps all three; `jdeprscan`/`jmod`/`lsof`'s `-?, -h, --help` keep all three; `dpkg`'s `--no-act, --dry-run, --simulate` keeps all three. A tool's own `-r[esolve]`-style abbreviation bracket (a prefix a user may type, glued to the rest of the word it abbreviates — one letter or more, short or long dashes: `ip`'s `-r[esolve]`/`-rc[vbuf]`, `--br[ief]`) is now read as one spelling naming the full word, rendered back exactly as documented, rather than either discarding the bracket or misreading it as a fabricated value. It also now folds adjacent option-table rows into one entity when a tool spells one flag across *several separate rows* rather than one — the same issue's own primary example, `ffplay --help`'s `-h topic` / `-? topic` / `-help topic` / `--help topic` (four rows, one spelling apiece), now reads as the single entity `-h, -?, -help, --help  topic  show help` instead of splitting across two or three, with `-help`'s own `topic` value restored from its run-mates' agreement rather than left cleared by the single-dash long-option repair. The fold is strict — same table, one spelling per row, identical description/value shape, and at most one distinct long-like name in the run — so two genuinely different long options that merely share a boilerplate description (`dbiprof`'s `-match=K=V`/`-exclude=K=V`, `dpkg`'s `--configure`/`--triggers-only`) are never merged.
