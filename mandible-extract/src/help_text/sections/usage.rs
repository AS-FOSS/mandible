...
/// specimen ("Create a new LV, then attach a
/// cachevol created from" / "the specified cache device, which converts
/// the" / "new LV to type cache.") is three, so four leaves one line of
/// headroom without admitting an unrelated multi-paragraph run. See
...
/// ([`recover_stanza_head_flag`]'s narrower single-flag shape, S-089) or
/// several required flags glued into one whole invocation form
/// (`lvcreate -m|--mirrors Number -L|--size Size[m|UNIT] VG`).
/// [`stanza_description_above`] only needs to know the line reads as an
/// invocation; whether a single mode-flag entity can *also* be built from
/// it is `recover_stanza_head_flag`'s own, unrelated question. See
/// docs/shapes.md S-137.
...
/// True when `line` (already known non-blank) may extend a stanza
/// description run: sits at `indent`, is a single field
/// ([`find_multi_space_gap`]), and opens with neither the tool's own
/// name, flag/usage notation, nor an [`is_ignorable_heading`] marker. See
/// S-012, S-137.
fn extends_stanza_description(line: &str, indent: usize, name: &str) -> bool {
    if leading_whitespace(line) != indent || find_multi_space_gap(line).is_some() {
        return false;
    }
    let text = line.trim();
    !text.is_empty()
...
/// usage stanza's head line, when `lines[head_idx]` is such a head and one
/// or more lines above it, joined, read as such a sentence — LVM's own
/// one-stanza-per-form emitter:
///
/// ```text
...
/// ```
///
/// and, wrapped across three lines, lvcreate's own cachevol form:
///
/// ```text
...
/// ```
///
/// The head must be [`looks_like_invocation_form_head`]-shaped. The block
/// above it (at most [`MAX_STANZA_DESCRIPTION_LINES`] lines) must: sit at
/// the head's own column; stand alone (its own predecessor blank or
/// absent — the anti-paragraph clause, so a hard-wrapped or trailing-
/// paragraph sentence is refused whole rather than adopted as a
/// fragment); each line pass [`extends_stanza_description`]; every line
/// but the last carry the raw trailing space its own emitter's word wrap
/// leaves behind — real evidence of one wrapped sentence, since two
/// merely-adjacent complete-looking lines (an unrelated paragraph's own
/// last sentence, `trailing_line_of_a_paragraph_is_not_adopted_as_a_stanza_label`)
/// carry none; and, joined with single spaces, end in a full stop (not an
/// ellipsis) and carry at least [`MIN_STANZA_DESCRIPTION_WORDS`] words.
/// Returned verbatim, terminator included — the display layer strips it
/// (spec §9.3). See S-012, S-137.
pub(super) fn stanza_description_above(
    ...
    while start > floor && extends_stanza_description(lines[start - 1], head_indent, name) {
        start -= 1;
        // Every line but the block's last (which naturally ends the
        // sentence with its own period, needing no wrap after it) must
        // carry the raw trailing space a genuine hard wrap leaves — see
        // this function's own doc comment.
        if start + 1 < head_idx && !lines[start].ends_with(' ') {
            return None;
        }
    }
