...
//! A usage label and the tool's own name at a synopsis head are the same
//! question asked of a labelled line, so their predicates live here too.
...
pub(super) fn starts_attested_headingless_flag_block(lines: &[&str], idx: usize) -> bool {
    ...
    let (_, entries, _, _, _, _) = scan_flags_block(lines, idx, false);
